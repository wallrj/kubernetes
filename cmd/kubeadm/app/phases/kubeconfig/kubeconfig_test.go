...
func TestCreateKubeConfigAndCSR(t *testing.T) {
	...
	tests := []testCase{
		...
		{
			name: "error nil kubeconfigspec",
			setup: func(t *testing.T, tc *testCase) {
				tc.kubeConfigSpec = nil
			},
			expectedError: errInvalid,
		},
		{
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			test.kubeConfigSpec = &kubeConfigSpec{
			...
			if test.assertion == nil && test.expectedError == nil {
				test.assertion = func(t *testing.T, tc *testCase) {
					assert.FileExists(t, tmpDir+"/"+tc.kubeConfigName)
					assert.FileExists(t, tmpDir+"/"+tc.kubeConfigName+".csr")
				}
			}
